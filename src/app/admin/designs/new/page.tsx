...
export default function AddDesignPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (imageFiles.length === 0) { toast.error('Add at least one image'); return; }
    ...
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Fill in the details and upload images</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-white/5">
          <h2 className="text-white font-semibold mb-4">Design Images <span className="text-gray-500 text-xs font-normal">(up to 5)</span></h2>
          <div className="flex flex-wrap gap-3 mb-3">
            {imagePreviews.map((src, i) => (
              <div key={i} className="relative w-24 h-32 rounded-xl overflow-hidden bg-gray-800">
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-white/5">
          ...
          <div className="flex flex-wrap gap-2">
            {FABRICS.map(fabric => (
              <button
                key={fabric} type="button" onClick={() => toggleFabric(fabric)}
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={uploading}
            className="flex-1 py-3 bg-gradient-to-r from-rose-600 to-pink-600 text-white font-semibold text-sm rounded-xl hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
            {uploading ? (
              <><div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Uploading...</>
            ) : (
              <><Upload className="w-4 h-4" /> Publish Design</>
            )}
          </button>
        </div>
      </form>
