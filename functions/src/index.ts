...
export const onNewContactMessage = functions.firestore
  ...
  .onCreate(async (snap) => {
    ...
    if (!data) return;
    try {
      const priorityEmoji: Record<string, string> = { urgent: '🚨', high: '⚠️', medium: '📩', low: '📬' };
    ...
    } catch (error) { console.error('Contact message email error:', error); }
  });

// ====== TRIGGER: Admin Reply to Contact Message ======
export const onContactMessageReply = functions.firestore
  .document('contactMessages/{msgId}')
  .onUpdate(async (change) => {
    ...
    if (newReplies <= prevReplies) return; // No new reply added
    const latestReply = after.replies[newReplies - 1];
    if (!latestReply || !after.email) return;
    try {
      await transporter.sendMail({
        from: `"Nilkanth Fashions" <${ADMIN_EMAIL}>`,
        to: after.email,
...
export const onNewCustomDesignRequest = functions.firestore
...
export const onDesignRequestStatusUpdate = functions.firestore
  .document('customDesignRequests/{requestId}')
    ...
    try {
      await transporter.sendMail({
        ...
        html: `
          ...
          <p>Items: ${data.items?.map((i: any) => i.designName).join(', ')}</p>
...
export const getAnalyticsSummary = functions.https.onCall(async (data, context) => {
  ...
  if (userDoc.data()?.role !== 'admin') throw new functions.https.HttpsError('permission-denied', 'Admin only');
  ...
  const popularDesigns: Record<string, number> = {};
